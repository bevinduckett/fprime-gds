...
class DataProductDecoder:
    ...
    def decode_header(self, file_handle) -> Dict[str, Any]:
        ...
        if header.to_jsonable()["Checksum"]["value"] != computed_hash:
            raise CRCError(
                "Header", header.to_jsonable()["Checksum"]["value"], computed_hash
            )
    ...
    def decode_record(self, file_handle, record_id: int) -> Dict[str, Any]:
        ...
        record_template: DpRecordTemplate = self.dictionaries.dp_record_id.get(
            record_id
        )
        ...
        if record_template.get_is_array():
            ...
            element_array_type = ArrayType.construct_type(
                f"{record_template.get_name()}_{array_size}",
                record_type,
                array_size,
                "{}",
            )
        ...
        while (r_io.tell() - position_at_start) < data_size:
            ...
            if len(record_id_bin) == 0:
                ...
                raise DataProductError(
                    f"End of file reached while processing data product"
                )
    ...
    def is_compression_record(self, record):
        return (
            re.search(
                "dpCompressProc.CompressionRecord$", record["Record"]["record_name"]
            )
            is not None
        )
    ...
    def decompress_records(self, records):
        ...
        for record in records:
            ...
            if record_meta.val["algorithm"] == "UNCOMPRESSED":
                ...
                raise DataProductError(
                    f"Compression algorithm {record_meta.val['algorithm']} unsupported"
                )
    ...
    def decode(self) -> List[Dict[str, Any]]:
        ...
        if (
            not self.disable_decompression
            and len(results["Records"]) > 0
            and self.is_compression_record(results["Records"][0])
        ):
