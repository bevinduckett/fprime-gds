    ...
    def _await_completion(
        ...
        while time.time() < end_time:
            ...
            if entry.get("state") == SUCCESS_STATE:
                cls._log(
                    f"All uplink packets sent for '{source_name}' to '{entry.get('destination')}'"
                )
                return True
            if entry.get("state") in FAILURE_STATES:
                cls._log(
                    f"Uplink of '{source_name}' failed with state {entry.get('state')}"
                )
                return False
            time.sleep(POLL_PERIOD_SECONDS)
        cls._log(
            f"Error: uplink of '{source_name}' did not complete within {args.timeout} seconds"
        )
        return False
