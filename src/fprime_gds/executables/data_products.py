...
def main():
    ...
    decode_parser = subcommands_parser.add_parser(
        "decode", help="Decode a data product binary into a human-readable format"
    )
    decode_parser.add_argument(
        "-b",
        "--bin-file",
        required=True,
        help="Path to input data product binary file (.fdp)",
    )
    decode_parser.add_argument(
        "-d", "--dictionary", required=True, help="Path to F Prime JSON Dictionary"
    )
    decode_parser.add_argument(
        "-o",
        "--output",
        required=False,
        help="Path to output JSON file (defaults to <binFilename>.json)",
    )
    decode_parser.add_argument(
        "-z",
        "--disable-decompression",
        action="store_true",
        help="Disable automatic decompression of data products",
    )

    validate_parser = subcommands_parser.add_parser(
        "validate", help="Validate a data product"
    )
    ...
    validate_parser.add_argument(
        "-d", "--dictionary", required=False, help="Path to F Prime JSON Dictionary"
    )
    validate_parser.add_argument(
        "-s",
        "--header-size",
        type=int,
        default=0,
        help="Use the provided value as the header size for the data product",
    )
    validate_parser.add_argument(
        "-g", "--guess-size", action="store_true", help="Guess at the header size"
    )
    validate_parser.add_argument(
    ...
    if args.command == "decode":
        ...
        DataProductDecoder(
            args.dictionaries, args.bin_file, args.output, args.disable_decompression
        ).process()
