...
from fprime_gds.common.dp.decoder import (
    DataProductDecoder,
    CRCError,
    RecordNotFoundError,
)
...
class TestDataProductDecoderProcessMethod:
    ...
    def test_process_writes_json_file(self, load_dictionary, tmp_path):
        ...
        decoder = DataProductDecoder(
            load_dictionary, str(TEST_DATA_DIR / "makeBool.bin"), str(output_path)
        )
        decoder.process()
    ...
    def test_process_default_output_path(self, load_dictionary, tmp_path):
        ...
        decoder = DataProductDecoder(load_dictionary, str(test_bin))
        decoder.process()
...
class TestDataProductDecoderErrorHandling:
    ...
    def test_decode_file_too_small(self, load_dictionary, tmp_path):
        ...
        decoder = DataProductDecoder(
            load_dictionary, str(small_file), str(tmp_path / "output.json")
        )
...
class TestDataProductDecoderRecordDecoding:
    ...
    def test_decode_header(self, load_dictionary):
        ...
        decoder = DataProductDecoder(
            load_dictionary, str(TEST_DATA_DIR / "makeBool.bin")
        )
    ...
    def test_decode_scalar_record(self, load_dictionary):
        ...
        decoder = DataProductDecoder(
            load_dictionary, str(TEST_DATA_DIR / "makeU32.bin")
        )
        result = decoder.decode()
    ...
    def test_decode_array_record(self, load_dictionary):
        ...
        decoder = DataProductDecoder(
            load_dictionary, str(TEST_DATA_DIR / "makeU8Array.bin")
        )
        result = decoder.decode()
...
class TestDataProductDecoderIntegration:
    ...
    def test_decode_all_test_files(self, load_dictionary, tmp_path):
        ...
        for test_file in valid_files:
            ...
            if file_path.exists():
                decoder = DataProductDecoder(
                    load_dictionary, str(file_path), str(tmp_path / f"{test_file}.json")
                )
                result = decoder.decode()
                ...
                assert (
                    "Header" in result
                ), f"Failed to decode {test_file}: missing Header"
                assert (
                    "Records" in result
                ), f"Failed to decode {test_file}: missing Records"
                assert (
                    len(result["Records"]) > 0
                ), f"Failed to decode {test_file}: no records"
...
class TestDataProductDecoderCompressedProducts:
    ...
    def test_decode_compressed(self, load_dictionary_ref, tmp_path):
        ...
        decoder = DataProductDecoder(
            load_dictionary_ref, str(TEST_DATA_DIR / "DpDemoCompressed.fdp")
        )
        result = decoder.decode()
    ...
    def test_decode_compressed_raw(self, load_dictionary_ref, tmp_path):
        ...
        assert (
            result["Records"][0]["Record"]["record_name"]
            == "DpCompression.dpCompressProc.CompressionRecord"
        )
    ...
    def test_decode_compressed_compare(self, load_dictionary_ref, tmp_path):
        ...
        decoder_uncompressed = DataProductDecoder(
            load_dictionary_ref, str(TEST_DATA_DIR / "DpDemoUncompressed.fdp")
        )
