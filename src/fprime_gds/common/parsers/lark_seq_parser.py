...
class LarkSeqFileParser:
    ...
    def parse(
        self, filename: str, cont: bool = False
    ) -> Generator[Tuple[int, Descriptor, int, int, str, List[Any]], None, None]:
        """
        ...
        try:
            ...
            col_num = e.column
            msg = (
                f"{filename_abs}:{line_num}:{col_num}: Encountered syntax error parsing"
            )
    ...
    def _parse_time_tag(
        self, time_tag: Tree[Token], line_number: int
    ) -> Tuple[Descriptor, int, int]:
        """
