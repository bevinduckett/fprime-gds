...
class ChainedFramerDeframer(FramerDeframer, ABC):
    ...
    def get_composites(cls) -> List[Type[FramerDeframer]]:
        ...
        raise NotImplementedError(
            f"Subclasses of {cls.__name__} must implement get_composites"
        )

    @staticmethod
    def get_argument_subset(
        composite: Type[FramerDeframer], argument_dictionary: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        needed_argument_destinations = [
            (
                ...
                else [dash_dash for dash_dash in flag if dash_dash.startswith("--")][0]
                .lstrip("-")
                .replace("-", "_")
            )
        ...
        return {
            name: argument_dictionary[name] for name in needed_argument_destinations
        }
    ...
    def get_arguments(cls):
        ...
        for composite in cls.get_composites():
            all_arguments.update(
                composite.get_arguments() if hasattr(composite, "get_arguments") else {}
            )
    ...
    def deframe_all(self, data, no_copy):
        ...
        for deframer in self.deframers:
            ...
            for packet_data in packets:
                ...
                new_packets, new_remaining, new_discarded = deframer.deframe_all(
                    packet_data, True
                )
    ...
    def frame(self, data):
        ...
        return reduce(
            lambda framed_data, framer: framer.frame(framed_data), self.framers, data
        )
...
class SpacePacketSpaceDataLinkFramerDeframer(ChainedFramerDeframer):
    ...
    def get_composites(cls) -> List[Type[FramerDeframer]]:
        ...
        Innermost FramerDeframer should be first in the list."""
        return [SpacePacketFramerDeframer, SpaceDataLinkFramerDeframer]
