...
class TestNameMapping:
    ...
    def test_to_fprime_name_no_namespace(self):
        ...
        assert (
            wrapper.to_fprime_name("/Ref/blockDrv/BD_Cycles")
            == "Ref.blockDrv.BD_Cycles"
        )
    ...
    def test_to_yamcs_qualified_arg(self):
        wrapper = self.make_wrapper()
        assert (
            wrapper.to_yamcs_qualified_arg(
                "/MyDeployment/Ref/cmdDisp/CMD_NO_OP_STRING", "arg1"
            )
...
class TestParseUri:
    def test_plain_host_port(self):
        assert (
            YamcsClient._parse_uri("yamcs://localhost:8090") == "http://localhost:8090"
        )
    ...
    def test_trailing_path_stripped(self):
        assert (
            YamcsClient._parse_uri("yamcs://host:8090/some/path") == "http://host:8090"
        )
...
class TestDiscoverInstanceAndProcessor:
    ...
    def test_falls_back_to_first_processor(self, mocker):
        ...
        self.make_lib_client(mocker, [self.named("a", "RUNNING")], processors)
        _, processor = YamcsClient._discover_instance_and_processor(
            "http://localhost:8090"
        )
