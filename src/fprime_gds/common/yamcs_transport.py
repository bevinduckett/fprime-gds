...
class YamcsWrapper:
    ...
    def configure(self, yamcs_url, instance, processor_name):
        ...
        self.processor = self.yamcs_client.get_processor(
            instance=instance, processor=processor_name
        )
...
class YamcsClient(TransportClient):
    ...
    def _issue_command(self, cmd_data):
        ...
        try:
            ...
            LOGGER.info(
                "Command issued: %s id=%s", yamcs_cmd_name, getattr(issued, "id", None)
            )
        except Exception as exc:
    ...
    def _build_ch_data(self, param_value):
        ...
        if param_value.eng_value is None:
            return None
        val_obj = self._build_value_object(
            param_value.eng_value, template.get_type_obj()
        )
    ...
    def _build_event_data(self, event):
        template = self.dictionaries.event_name.get(
            event.event_type
        ) or self._event_by_leaf.get(event.event_type)
        if template is None:
        ...
        if not template_args:
            return EventData(
                tuple(), self._build_time_type(event.generation_time), template
            )
        ...
        return EventData(
            tuple(arg_objs), self._build_time_type(event.generation_time), template
        )
    ...
    def upload_file(
        ...
        local_path,
        remote_path,
        bucket_name=FILE_TRANSFER_BUCKET,
        service_name=FILE_TRANSFER_SERVICE_NAME,
        timeout=120,
    ):
        ...
        LOGGER.info(
            "Staged %s in bucket %s as %s", local_path, bucket_name, object_name
        )
        ...
        LOGGER.info(
            "Upload transfer started: id=%s remote=%s", transfer.id, remote_path
        )
    ...
    def download_file(
        ...
        if object_name is None:
            object_name = (
                remote_path.split("/")[-1] if "/" in remote_path else remote_path
            )
        ft_service = self.yamcs.get_file_transfer_service(service_name)
        ...
        LOGGER.info(
            "Download transfer started: id=%s remote=%s", transfer.id, remote_path
        )
        return self._await_transfer(ft_service, transfer, timeout)
    ...
    def _await_transfer(ft_service, transfer, timeout):
        ...
        raise TransportationException(
            f"File transfer {transfer.id} timed out after {timeout} seconds"
        )
    ...
    def _discover_instance_and_processor(yamcs_url):
        ...
        if not processors:
            raise RuntimeError(
                "No processors found for YAMCS instance %s at %s"
                % (instance, yamcs_url)
            )
        ...
        LOGGER.info(
            "Auto-discovered YAMCS instance=%s, processor=%s", instance, processor_name
        )
        return instance, processor_name

    def _build_parameter_list(self):
        if self.dictionaries is None or not getattr(
            self.dictionaries, "channel_name", None
        ):
            LOGGER.warning(
                "No channel dictionary available; subscribing to no parameters"
            )
            return []
        return [
            self.yamcs.to_yamcs_param_name(name)
    ...
    def _build_time_type(yamcs_timestamp):
        ...
        if yamcs_timestamp.tzinfo is not None:
            yamcs_timestamp = yamcs_timestamp.astimezone(datetime.timezone.utc).replace(
                tzinfo=None
            )
        time_obj = TimeType()
